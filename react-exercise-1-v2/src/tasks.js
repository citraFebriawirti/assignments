...
function Task1() {
  return <div className="basic_component">
<h1>Halo dunia!</h1>
  </div>
}
...
function Task2() {
  ...
  return (
    <div>
      <h1>{name}</h1> 
      <button onClick={() => setName("Tombol telah di-klik!")}>Klik saya!</button>
    </div>
  );
}
...
function Task3_1() {
  return (
    <div>
      {students.filter(el=> el.dropout === false).map((murid) => (
        <div>
          <h2>{murid.name} - {murid.age}</h2>
        </div>
      ))}
