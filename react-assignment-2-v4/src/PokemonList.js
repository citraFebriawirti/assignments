...
const Pagination = () => {
  ...
  const currentPage = +(searchParams.get("page") || 1); // menginisialisasikan halaman sekarang

  const moveTo = (direction) => {
    if (direction === "prev") {
      // set dengan currentPage sebelumnya
      setSearchParams({ page: currentPage - 1 });
    } else {
      // set dengan currentPage sesudahnya
      setSearchParams({ page: currentPage + 1 });
    }
  };
  ...
  return (
    <HStack>
      ...
      <Button onClick={() => moveTo("prev")} disabled={currentPage === 1}>
        Prev
      </Button>
        ...
        {pokemons.map((pokemon) => (
          <Link key={pokemon.id} to={`/pokemon/${pokemon.id}`}>
            <Card>
              ...
              <CardBody>
                {/* TODO: render pokemon images & type 
              here */}
                <HStack>
                  <Image
